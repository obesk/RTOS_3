...
int main() {
	...
	// superuser). Check that the main thread is executed with superuser
	// privileges before doing anything else.

	if (getuid() == 0)
		pthread_setschedparam(pthread_self(), SCHED_FIFO, &priomax);

	// execute all tasks in standalone modality in order to measure execution
	// times (use gettimeofday). Use the computed values to update the worst
	// case execution time of each task.
	...
	for (i = 0; i < NTASKS; i++) {
		...
		// compute the Worst Case Execution Time (in a real case, we should
		// repeat this many times under
	...
	// compute U
	double U =
		WCET[0] / periods[0] + WCET[1] / periods[1] + WCET[2] / periods[2];

	// compute Ulub by considering the fact that we have harmonic relationships
	// between periods
	double Ulub = 1;
	...
	// set the minimum priority to the current thread: this is now required
	// because
	...
	for (i = 0; i < NPERIODICTASKS; i++) {
		...
		// policies are explicitly chosen, not inherited from the main thread
		...
		// properly set the parameters to assign the priority inversely
		// proportional to the period parameters[i].sched_priority =
	...
	// set the next arrival time for each task. This is not the beginning of the
	// first period, but the end of the first period and beginning of the next
	// one.
	for (i = 0; i < NPERIODICTASKS; i++) {
		long int next_arrival_nanoseconds = time_1.tv_nsec + periods[i];
		// then we compute the end of the first period and beginning of the next
		// one
		next_arrival_time[i].tv_nsec = next_arrival_nanoseconds % 1000000000;
		next_arrival_time[i].tv_sec =
			time_1.tv_sec + next_arrival_nanoseconds / 1000000000;
...
void *task1(void *ptr) {
	...
	// execute the task one hundred times... it should be an infinite loop (too
	// dangerous)
	int i = 0;
	for (i = 0; i < 100; i++) {
		...
		// sleep until the end of the current period (which is also the start of
		// the new one
		clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &next_arrival_time[0],
						NULL);
		...
		long int next_arrival_nanoseconds =
			next_arrival_time[0].tv_nsec + periods[0];
...
void *task2(void *ptr) {
	...
	for (i = 0; i < 100; i++) {
		...
		clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &next_arrival_time[1],
						NULL);
		long int next_arrival_nanoseconds =
			next_arrival_time[1].tv_nsec + periods[1];
...
void *task3(void *ptr) {
	...
	for (i = 0; i < 100; i++) {
		...
		clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &next_arrival_time[2],
						NULL);
		long int next_arrival_nanoseconds =
			next_arrival_time[2].tv_nsec + periods[2];
